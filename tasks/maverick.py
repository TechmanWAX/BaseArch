...
class Maverick(Base):
    PATH_MAP_BUY = {
        'USDC': [
                AsyncWeb3.to_bytes(hexstr=HexStr(Contracts.WETH.address)),
                AsyncWeb3.to_bytes(hexstr=HexStr('41c8cf74c27554a8972d3bf3d2bd4a14d8b604ab')),
                AsyncWeb3.to_bytes(hexstr=HexStr(Contracts.USDC.address)),
            ],
        'BUSD': [
                AsyncWeb3.to_bytes(hexstr=HexStr(Contracts.WETH.address)),
                AsyncWeb3.to_bytes(hexstr=HexStr('57681331b6cb8df134dccb4b54dc30e8fcdf0ad8')),
                AsyncWeb3.to_bytes(hexstr=HexStr(Contracts.USDC.address)),
                AsyncWeb3.to_bytes(hexstr=HexStr('e799043fb52ff46cc57ce8a8b1ac3f151ba270f7')),
                AsyncWeb3.to_bytes(hexstr=HexStr(Contracts.ceBUSD.address))
            ],
    }
    PATH_MAP_SELL = {
        'USDC': [
        ...
        'BUSD': [
                AsyncWeb3.to_bytes(hexstr=HexStr(Contracts.ceBUSD.address)),
                AsyncWeb3.to_bytes(hexstr=HexStr('3ae63fb198652e294b8de4c2ef659d95d5ff28be')),
                AsyncWeb3.to_bytes(hexstr=HexStr(Contracts.WETH.address)),
            ],
    }
    ...
    async def _swap(
        ...
        to_token_address = AsyncWeb3.to_checksum_address(path[-1])
        ...
        from_token_address = AsyncWeb3.to_checksum_address(path[0])
